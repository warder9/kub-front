    ...
    const fetchUserData = async () => {
      try {
        ...
        const roleNames: Record<number, string> = {
          ...
          15: 'Административный персонал',
          10: 'Отдел продаж',
        };
        setRoleName(role?.name || roleNames[userData.role_id] || 'Пользователь');
      } catch (error) {
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-slate-50 rounded-xl p-4">
                ...
                <p className="font-semibold text-slate-900">{user.bin_iin || 'Не указан'}</p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-slate-50 rounded-xl p-4">
                ...
                <p className="font-semibold text-slate-900">
                  {user.telegram_chat_id ? user.telegram_chat_id : 'Не подключен'}
                </p>
              </div>
              <div className="bg-slate-50 rounded-xl p-4">
                <p className="text-sm text-slate-600 mb-1">Уведомления о задачах</p>
                <p className="font-semibold text-slate-900">
                  {user.notify_tasks_telegram ? 'Включены' : 'Выключены'}
                </p>
              </div>
            </div>
          </div>
